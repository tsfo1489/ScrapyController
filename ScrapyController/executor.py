...
def json_to_cmd(json_data):
    ...
    cmds = base + [json_data['spider']] + parameters + options + [f'-o ../../data/{json_data["file"]}']
    cmd = ' '.join(cmds)
    ...
    return cmd, './crawler/' + json_data['path'], final_log

def executor(json_data: dict):
    cmd, path, log_setting = json_to_cmd(json_data)
    open('./log/' + log_setting['TEMP_LOG'], 'w').close()
    print(cmd.split())
    proc = subprocess.Popen(cmd.split(), cwd=path, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
