...
function ChangeCycle(){
  var cycle_radio = $('input[name=CycleOptions]:checked').val();
  console.log(cycle_radio);
  if(cycle_radio == 'day'){
    $('#DaysText').attr('disabled', true);
  }
  else{
    $('#DaysText').attr('disabled', false);
  }
}
...
function Submit(){
  ...
  new_sch.cycle = $('input[name=CycleOptions]:checked').val();
  new_sch.day = $('#DaysText').val().split(',');
  if(new_sch.day == '' && new_sch.cycle != 'day')
  ...
  new_sch.time = $('#TimeText').val().split(',');
  if(new_sch.time == '')
  ...
  console.log(new_sch);
  $.ajax({
    url: '/schedules/' + $('#ScheduleName').val(),
    method: "POST",
